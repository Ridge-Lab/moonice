...
- [CI 35725800115](https://github.com/Ridge-Lab/moonice/actions/runs/35725800115)
  passed all four targets including native, all examples, web build and deploy.
- Registry-only independent consumers downloaded `Ridge-Lab/moonice@0.2.0`:
  JS MoonFrame integration and Wasm standalone-core batch execution passed.
  The combined MoonFrame dependency graph fails on Wasm due to x 0.4.47's
  `IOError` interface versus Parquet 0.2.1. This example is documented as JS-only;
  the standalone core resolves x 0.4.40 and passes Wasm and the four-target CI.
